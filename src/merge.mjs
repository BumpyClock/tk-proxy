...
function sumTokenValues(tokens) {
...
export function isTokenContributionData(value) {
  return (
    isObject(value) &&
    isObject(value.meta) &&
...
export function extractPayloadFromJson(data) {
  if (isTokenContributionData(data)) {
    return data;
  }
  if (isObject(data) && isTokenContributionData(data.submitPayload)) {
    return data.submitPayload;
  }
  if (isObject(data) && isTokenContributionData(data.parsedStdout)) {
    return data.parsedStdout;
  }
  return null;
}

function collectRows(payloads) {
...
function getIntensity(cost, maxCost) {
...
function computeYearSummaries(contributions) {
  const years = new Map();
...
function buildContributions(byDate) {
